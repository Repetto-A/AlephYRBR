...
| Criterio del track | Cómo lo cubre ClinicGuard |
| --- | --- |
...
en `clinicguard/rules.py` · agente HCE en `clinicguard/propose.py` (sin LLM).
...
consultorio (agenda + HC + grabar + draft + propuesta HCE + aprobar). Sin
cloud, sin login.
...
`GET /api/config` · `GET /api/cases` · `POST /api/run` con
`{"case":"a"|"b","source":"audio"|"transcript"}` · `GET /api/run/{run_id}`
...
2. `python -m clinicguard serve --fast` → abrir `http://127.0.0.1:8787`.
...
→ reglas deterministas (`clinicguard/rules.py`) → propuesta HCE
(`clinicguard/propose.py`) → JSON + HTML / UI.
